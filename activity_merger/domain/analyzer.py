...
DEFAULT_RULES = [
    Rule2(BUCKET_AFK_PREFIX + ".*", 1, to_string="afk").with_subrules(
        key="status", subrules=[Rule2("afk", AFK_RULE_PRIORITY).skip(), Rule2("not-afk", 1).placeholder()]
    ),
    Rule2(BUCKET_STOPWATCH_PREFIX + ".*", 0, to_string="stopwatch").with_subrules(
        key="label",
        subrules=[
            Rule2(".*", 0).with_subrules(
                key="running",
                subrules=[
...
class RuleResultsWindow:
    ...
    def to_str(self, debug=False) -> str:
        ...
        if debug:
            ...
            events_str = "\n  ".join(
                f"{seconds_to_timedelta(x.duration.total_seconds())} {event_data_to_str(x)}" for x in events.values()
            )
...
class ProblemReporter:
    ...
    def report_missed_rule(cur_interval):
        LOG.info(
            "Skipping %s because can't find rule for events in it:\n  %s",
            cur_interval,
    ...
    def report_placeholder_rule(rule, cur_interval):
        LOG.info(
            "%s: Need to reveal rule for interval %s with %d events:\n  %s",
            rule,
            cur_interval.to_str(only_time=True),
            len(cur_interval.events),
            "\n  ".join(str(x) for x in cur_interval.events),
        )
...
def _window_to_activity(
    window: RuleResultsWindow,
    rule_result: RuleResult,
    activities: List[Activity],
    is_build_debug_buckets: bool,
    ...
    if is_build_debug_buckets:
        debug_event = Event(
            DEBUG_BUCKET_PREFIX + "3_activities",
            activity.start_time,
            activity.duration,
            {
...
def analyze_intervals(
    interval: Interval, round_to: float, custom_rules: List[Rule2], is_build_debug_buckets: bool = False
) -> AnalyzerResult:
    """
    ...
    while cur_interval.next:
        ...
        is_not_afk = any(
            x for x in cur_interval.events if x.bucket_id.startswith(BUCKET_AFK_PREFIX) and x.data["status"] != "afk"
        )
        if is_not_afk:
        ...
        if is_build_debug_buckets:
            debug_event = Event(
                BUCKET_DEBUG_RAW_RULE_RESULTS,
                cur_interval.start_time,
                cur_interval.get_duration(),
                {
                    "description": rule_result.description,
        ...
        if rule_result.rule.is_skip:
            LOG.debug(
                "Skipping %f sec %d interval(s) because of %s priority is highest for %s.",
                round(duration, 1),
                len(rule_result.intervals),
                rule_result.rule,
                rule_result.event,
            )
        ...
        if rule_result.rule.is_placeholder:
            metrics.increment_and_call_handler(
                "intervals need to reveal rule for", cur_interval, rule_result.rule, cur_interval
            )
        ...
        if is_build_debug_buckets:
            debug_event = Event(
                BUCKET_DEBUG_FINAL_RULE_RESULTS,
                rule_result.intervals[0].start_time,
        ...
        if window is not None:
            ...
            if _is_new_activity(window, rule_result):
                _window_to_activity(
                    window, rule_result, activities, is_build_debug_buckets, activity_debug_events, round_to, metrics
                )
                window = None
            else:
                ...
                if window.duration > TOO_LONG_ACTIVITY_ALERT_AFTER_SECONDS:
                    metrics.increment_and_call_handler(
                        "intervals matched by too wide rules", cur_interval, window, rule_result
                    )
